...
function updateNewsData() {
    ...
    $.get("/news_list", params, function (response) {
        data_querying = false;
        if (response.errno == 0){
            total_page = response.context.total_page;
            if (cur_page==1) {
                $(".list_con").html("");
            }
            cur_page += 1;
            for (var i=0; i<response.context.news_dict_list.length; i++ ) {
                var news = response.context.news_dict_list[i];
                var content = "<li>";
